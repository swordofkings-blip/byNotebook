...
> **Google NotebookLM × AIエージェントで創る、設定破綻を極限まで抑えた対話型ライトノベル生成環境**

AIロールプレイや対話小説でよくある**「途中で設定を忘れる」「キャラ崩壊」「不自然な知ったかぶり（全知化）」「オウム返しの多用」**を、Google NotebookLMの強力なRAG技術と専用プロンプトアーキテクチャによって大幅に改善した対話型ノベル環境です。

---

## 🌟 主な特徴

* 🧠 **設定忘れ・キャラ崩壊の防止**: NotebookLM（Gemini）の超高精度RAGにより、長編ストーリーでも設定や過去の約束を忠実に再現。
* 🎭 **知ったかぶり（全知化）の防止**:
