...
2. `worlds/_template/` を参考に、ユーザーの要望に合わせた以下のファイルを作成する：
   - `system_prompt.md`（以下の必須ルールを盛り込んだチャット指示）:
     ...
     - **NPCの潜伏アジェンダ・生活感シミュレーションと文末ツールチップ記録**:
       - 不在時の足取り・生活感を持たせ、各NPCに1つ以上の短期動機・葛藤を常時シミュレート。外面的な身体言語（演技）としてのみ表出。
       - 思考のサボりや時系列喪失を防止するため、小説本文の末尾（最後の一行）にMarkdownリンクツールチップ形式（`[🔍](<#【◯日目・時間帯】...>)` ※NotebookLMがURLホバーをポップアップ表示する仕様を利用）で現在状況を毎ターン記録させる。
     - **ハプニング発生制御と空気読み**:
       - 世界観に応じた頻度設定（低/標準/高）。重要な告白や情緒的対話の最中は介入厳禁（空気を読む）。沈黙やループなど停滞時のみ起爆剤として投入
