...
## 9. AIに世界観・キャラクター設定を作ってもらう時のプロンプトテクニック

AIコーディングアシスタント（Cursor / Windsurf / Antigravity等）に新しい物語ノートの設定ファイル作成を依頼する際、プロンプトに以下のフレーズを添えるのが非常におすすめです。

### 「矛盾や確認事項があれば確認してください」と一言添える
* **効果**:
  AIがいきなり設定を決め打ちで作成せず、**「主人公やヒロインの年齢・立場」「二人の初期の関係性や距離感」「世界観の前提ルール」など、曖昧な点や確認が必要な箇所を事前に質問・確認するフェイズ**を挟んでくれます。
* **プロンプト入力例**:
  ```text
  「高校の放課後を舞台に、図書室で後輩の女の子と過ごす日常系の物語設定ファイルを作って。
  矛盾や確認事項があれば作成前に確認してください」
  ```
* **メリット**:
  AIからの質問に答えていく（対話ですり合わせを行う）ことで、自分でも言語化できていなかった細かなニュアンスや好みが明確になり、**より自身の望んだ通りの世界観・キャラクター設定**に仕上げることができます。

### 💡 （発展）対応環境なら `/grill-me` コマンドで強制インタビューさせる
Google Antigravityなどのスラッシュコマンド対応エージェントを使用している場合、依頼文に **`/grill-me`** を含めると、AIが即座に設定作成を行わず、**徹底的に質問攻め（インタビュー）をして設定を深掘りするモード**に入ります。
* **入力例**:
  ```text
  /grill-me 高校の図書室を舞台にした日常系の世界観を作りたい
  ```
* **効果**:
  「ヒロインの性格タイプは？」「二人の関係性の進展スピードは？」「周囲の人間関係は？」など、選択肢付きで多角的に逆質問してくるため、「アイデアはあるけれど設定を壁打ちしながら一緒に固めたい」という時に非常に強力です。
